...
from User.forms import UserRegister, FarmerProfileForm
...
class UserProfileView(LoginRequiredMixin, View):
    ...
    def get(self, request, username):
        ...
        form = FarmerProfileForm(instance=Farmer_Profile) if request.user == Farmer_Profile else None

        Context = {"farmer": Farmer_Profile, "form": form}
    ...
    def post(self, request, username):
        ...
        if request.user != Farmer_Profile:
            messages.error(request, "You are not allowed to edit this profile.")
            return redirect("Production:user-profile", username=Farmer_Profile.username)
        ...
        if action == "password":
            ...
            if not Farmer_Profile.check_password(current_password):
                ...
                Farmer_Profile.set_password(new_password)
                Farmer_Profile.save()
                logout(request)
                messages.success(
                    request,
...
def DeleteREgionalPrice(request, p_id):
    ...
    if d_price:
        d_price.delete()
        messages.error(
            request, f"regional price of {d_price.crop} has beeen deleted successfuly"
        )
...
class FarmDataAPIView(LoginRequiredMixin, APIView):
    def get(self, request):
        ...
        crop_output = farms.values("crop_type__name").annotate(
            total_output=Sum("total_output")
        )
        data = [
            {"crop_name": item["crop_type__name"], "total_output": item["total_output"]}
        ...
        summary = {
            ...
            "total_output": farms.aggregate(total_output=Sum("total_output"))[
                "total_output"
            ],
        }
        ...
        if form.is_valid():
            ...
            if not existing_instance:
                ...
                for e in existing_instance:
                    ...
                    messages.error(
                        request,
                        f"There is already regional price of {c_name} in  {r_name}",
                    )
                    return redirect("Production:regional-prices")
...
class FarmerDetailsView(LoginRequiredMixin, View):
    ...
    def get(self, request, f_username):
        ...
        context = {"farmer": owner, "form": form, "farms": farms}
        return render(request, self.template_name, context)
    ...
    def post(self, request, f_username):
        ...
        try:
            ...
            previous_verification_instance = OutputVerification.objects.filter(
                owner=farmer
            )
            ...
            OutputVerificationUtil.create_output_verification(
                owner=farmer, farm_name=farm_name, farm_output=output
            )
...
def dashboard(request):
    ...
    context = {"farmers": farmers, "regions": regions, "crops": crops, "farms": farms}
    return render(request, template_name, context)
...
def register(request):
    ...
    form = UserRegister()
    context = {"farmers": farmers, "regions": regions, "crops": crops, "form": form}
